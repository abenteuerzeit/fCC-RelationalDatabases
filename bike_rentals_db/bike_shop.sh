...
RENT_MENU() {
  # get available bikes
  AVAILABLE_BIKES=$($PSQL "SELECT bike_id, type, size FROM bikes
  WHERE available=true ORDER BY bike_id;")
  ...
  then
    ...
    then
      ...
      BIKE_AVAILABILITY=$($PSQL "SELECT available FROM bikes 
      WHERE bike_id='$BIKE_ID_TO_RENT' AND available=true;")
      # if not available
      ...
      SET_TO_FALSE_RESULT=$($PSQL "UPDATE bikes SET available=false 
    ...
    then
      ...
      then
        ...
        then
          ...
          SET date_returned=NOW() WHERE rental_id='$RENTAL_ID';")

          # set bike availability to true
          SET_TO_TRUE_RESULT=$($PSQL "UPDATE bikes 
          SET available=true WHERE bike_id='$BIKE_ID_TO_RETURN'")
